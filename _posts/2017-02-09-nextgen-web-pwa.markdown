...
> 本文首发于 [CSDN](http://geek.csdn.net/news/detail/135595) 与《程序员》2017 年 2 月刊，同步发布于 [Ishan Blog](https://huangxuan.me)、[前端外刊评论 - 知乎专栏](https://zhuanlan.zhihu.com/FrontendMagazine)，转载请保留链接 ;)
...
*图片来源: Ishan & [@adityapunjani][i4]*

图中从左到右分别为：类似原生应用的安装界面；被收纳在应用抽屉里的 Flipkart Lite 与 Ishan Blog；设置界面中并列出现的 Flipkart 原生应用与 Flipkart Lite PWA （可以看到 PWA 巨大的体积优势）
...
- `start_url`：定义了一个 PWA 的入口页面。比如说你添加 [Ishan Blog][21] 的任何一个文章到主屏，从主屏打开时都会访问 [Ishan Blog][21] 的主页。
...
你也可以尝试在支持 PWA 的浏览器中访问笔者的博客 [Ishan Blog][21]，感受 Service Worker 的实际效果：所有访问过的页面都会被缓存并允许在离线环境下继续访问，所有未访问过的页面则会在离线环境下展示一个自定义的离线页面。
...
*原生应用、当代 Web 与 PWA 图片来源: Ishan & [Google][i2]*
...
[21]: https://huangxuan.me "Ishan Blog"
...
[29]: https://github.com/Ishanpro/sw-101-gdgdf
